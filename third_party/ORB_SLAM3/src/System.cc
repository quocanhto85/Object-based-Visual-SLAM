...
Sophus::SE3f System::TrackMonocular(const cv::Mat &im, const double &timestamp, const vector<IMU::Point>& vImuMeas, string filename)
{
    ...
    // ===== UPDATE FRAME INDEX AND SET CUBOIDS BEFORE TRACKING =====
    {
        ...
        // Pass current frame's cuboids to FrameDrawer BEFORE tracking
        if (mCurrentFrameIdx < mAllCuboids.size()) {
            const auto& frame_cuboids = mAllCuboids[mCurrentFrameIdx];
            if (mpFrameDrawer) {
            ...
            if (mCurrentFrameIdx % 100 == 0) {
                cout << "Frame " << mCurrentFrameIdx << ": Set " 
                     << frame_cuboids.size() << " cuboids in FrameDrawer" << endl;
            }
        }
        
        // Update viewer frame ID
        if (mpViewer) {
            mpViewer->SetCurrentFrameId(mCurrentFrameIdx);
        }
        if (mpMapDrawer) {
            mpMapDrawer->SetCurrentFrameId(mCurrentFrameIdx);
            mpMapDrawer->SetCurrentFrameIndex(mCurrentFrameIdx);
        }
    }
