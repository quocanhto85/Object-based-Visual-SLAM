...
namespace ORB_SLAM3
{
                   ...
                   const bool bUseViewer, const int initFr, const string &strSequence) : mSensor(sensor), mpViewer(static_cast<Viewer *>(NULL)), mbReset(false), mbResetActiveMap(false), mbActivateLocalizationMode(false), mbDeactivateLocalizationMode(false), mbShutDown(false), mbCuboidExportEnabled(false), mCuboidOutputDir("")
    {
        ...
        {
            // Read camera intrinsics for FrameDrawer
            ...
            if (fSettings.isOpened())
            {
                ...
                // Fallback to Camera1 if Camera doesn't exist
                if (fx == 0.0f)
                {
                ...
                if (fx > 0.0f)
                {
                    ...
                    mpFrameDrawer->SetCameraMatrix(K);
                }
                else
                {
                    cerr << "WARNING: Could not read camera intrinsics from " << strSettingsFile << endl;
                }
                fSettings.release();
            }
            else
            {
                cerr << "WARNING: Could not open settings file: " << strSettingsFile << endl;
            }
        }
    ...
    Sophus::SE3f System::TrackMonocular(const cv::Mat &im, const double &timestamp, const vector<IMU::Point> &vImuMeas, string filename)
    {
        ...
        // ===== UPDATE FRAME INDEX AND SET CUBOIDS BEFORE TRACKING =====
        {
            unique_lock<mutex> lock(mMutexCuboids);

            // Get the actual tracking frame ID from the tracker
            unsigned long actualFrameId = mpTracker->mCurrentFrame.mnId;

            // Store cuboids for this frame using the ACTUAL frame ID
            if (mCurrentFrameIdx < mAllCuboids.size() && !mAllCuboids[mCurrentFrameIdx].empty())
            {
                vector<ORB_SLAM3::Cuboid> frame_cuboids = mAllCuboids[mCurrentFrameIdx];

                // Pass to FrameDrawer with current index (for display)
                mpFrameDrawer->SetCurrentFrameCuboids(frame_cuboids);

                // Pass to MapDrawer with ACTUAL frame ID
                if (mpMapDrawer)
                {
                    mpMapDrawer->SetCuboidsForFrame(actualFrameId, frame_cuboids);
                }

                // Debug output
                if (mCurrentFrameIdx % 100 == 0)
                {
                    cout << "Frame index " << mCurrentFrameIdx
                         << " -> Tracking ID " << actualFrameId
                         << ": Set " << frame_cuboids.size() << " cuboids" << endl;
                }
            }

            // Update viewer and MapDrawer frame ID with ACTUAL tracking ID
            if (mpViewer)
            {
                mpViewer->SetCurrentFrameId(actualFrameId);
            }
            if (mpMapDrawer)
            {
                mpMapDrawer->SetCurrentFrameId(actualFrameId);
            }
        }
        ...
        // ===== TRACK MONOCULAR - Same call for both MONOCULAR and IMU_MONOCULAR =====
        // Note: IMU measurements are handled internally by the tracker
        ...
        // INCREMENT frame counter AFTER tracking
        {
            unique_lock<mutex> lock(mMutexCuboids);
            mCurrentFrameIdx++;
        }
    ...
    // 3D_CUBOID
    void System::SetCuboids(const std::vector<std::vector<ORB_SLAM3::Cuboid>> &cuboids)
    {
        unique_lock<mutex> lock(mMutexCuboids);
        mAllCuboids = cuboids;
        mCurrentFrameIdx = 0; // Reset frame index

        cout << "=== System::SetCuboids ===" << endl;
        cout << "Loaded " << cuboids.size() << " frames of cuboid data" << endl;
        ...
        for (size_t i = 0; i < cuboids.size(); i++)
        {
            if (!cuboids[i].empty())
            {
                total += cuboids[i].size();
            }
        }
        cout << "Total cuboids: " << total << endl;

        // CRITICAL: Pass to Viewer
        if (mpViewer)
        {
            cout << "Passing cuboids to Viewer..." << endl;
            mpViewer->SetCuboids(cuboids);
        }
        else
        {
            cout << "WARNING: mpViewer is NULL!" << endl;
        }

        // Pass to MapDrawer
        if (mpMapDrawer)
        {
            cout << "Passing cuboids to MapDrawer..." << endl;
            mpMapDrawer->SetCuboids(cuboids);
        }
        else
        {
            cout << "WARNING: mpMapDrawer is NULL!" << endl;
        }

        cout << "=========================" << endl;
    }

    std::vector<ORB_SLAM3::Cuboid> System::GetCurrentFrameCuboids()
    {
        unique_lock<mutex> lock(mMutexCuboids);
        if (mCurrentFrameIdx < mAllCuboids.size())
        {
            return mAllCuboids[mCurrentFrameIdx];
        }
        return std::vector<ORB_SLAM3::Cuboid>();
    }
    ...
    void System::SaveKeyFrameTrajectoryKITTI(const string &filename, const string &filename2)
    {
        ...
        // Note: The original GitHub implementation had a monocular check that we removed
        // Original had: if(mSensor==MONOCULAR) { cerr << "ERROR..."; return; }
        // This modified version works with monocular SLAM
        ...
        for (size_t i = 0; i < vpKFs.size(); i++)
        {
            ...
            // Write KITTI format: r11 r12 r13 tx r21 r22 r23 ty r31 r32 r33 tz
            f << setprecision(9)
