...
namespace ORB_SLAM3
{
    ...
    void FrameDrawer::DrawCuboids2D(cv::Mat &im)
    {
        ...
        for (const auto &cub : mCurrentFrameCuboids)
        {
            ...
            if (valid_pts > 0)
            {
                ...
                string text;
                // Check if track_id is valid (positive and not default -1)
                bool has_track_id = (cub.track_id > 0);
                
                if (has_track_id) {
                    // ByteTrack case: show track_id
                    text = label + " " + to_string((int)(cub.confidence * 100)) + "% ID:" + to_string(cub.track_id);
                } else {
                    // Original cuboids case: no track_id
                    text = label + " " + to_string((int)(cub.confidence * 100)) + "%";
                }
    ...
    std::vector<std::vector<float>> FrameDrawer::GetCurrentCuboids()
    {
        unique_lock<mutex> lock(mMutexCuboids);
        ...
        for (const auto &cub : mCurrentFrameCuboids)
        {
            ...
            Eigen::Vector3f center = cub.center;
            Eigen::Vector3f dimensions = cub.dims;
            Eigen::Quaternionf rotation = cub.rot;

            // Get class information
            int class_id = cub.class_id;
            float confidence = cub.confidence;

            // Handle track_id
            // int track_id = -1; // Default if not available

            // If Cuboid struct has track_id field, uncomment below:
            int track_id = cub.track_id;

            // Pack into vector
            std::vector<float> cuboid_data = {
                center.x(),
                center.y(),
                center.z(),
                dimensions.x(),
                ...
                confidence,
                static_cast<float>(track_id)};

            cuboids.push_back(cuboid_data);
        }
