...
// void FrameDrawer::DrawCuboids2D(cv::Mat& im) {
//     // Note: mMutexCuboids is a member variable assumed to be defined in FrameDrawer.h
//     unique_lock<mutex> lock(mMutexCuboids);
    
//     if(mCurrentFrameCuboids.empty())
//         return;
    
//     // NOTE: We assume mK (camera calibration matrix) is a member variable (cv::Mat) and has been correctly set
//     // from the system's calibration file. If not, the projection will fail.
//     if(mK.empty() || mK.rows != 3 || mK.cols != 3) {
//         // cerr << "Warning: Camera matrix mK is not set for 2D cuboid projection." << endl;
//         return;
//     }

//     for(const auto& cuboid : mCurrentFrameCuboids) {
...
//         // Create 8 corners of the cuboid
//         vector<cv::Point2f> imgPoints;
//         vector<Eigen::Vector3f> corners(8);
...
//         // Transform to world coordinates (which is the camera frame here since cuboids are in camera frame) and project to image
//         for(auto& corner : corners) {
//             // Apply rotation and translation (object pose in camera frame)
//             corner = rotation * corner + center;
            
//             // Project to image (assuming pinhole camera model)
//             if(corner[2] > 0) { // Only project if in front of camera
//                 float u = K.at<float>(0,0) * corner[0] / corner[2] + K.at<float>(0,2);
//                 float v = K.at<float>(1,1) * corner[1] / corner[2] + K.at<float>(1,2);
//                 imgPoints.push_back(cv::Point2f(u, v));
//             } else {
//                 imgPoints.push_back(cv::Point2f(-1, -1)); // Invalid point
//             }
//         }
        
//         // Scale points if the image was scaled for display
//         float imageScale = 1.0f;
//         if (im.cols < mIm.cols) {
//             imageScale = (float)mIm.cols / im.cols;
//         }

//         for (cv::Point2f& p : imgPoints) {
//             if (p.x > 0) {
//                 p.x /= imageScale;
//                 p.y /= imageScale;
//             }
...
//         }
//         else if(cuboid.class_name == "Pedestrian" || cuboid.class_name == "pedestrian" || cuboid.class_name == "Person" || cuboid.class_name == "person") {
...
//         // Helper to draw a line only if both points are valid
//         auto draw_line = [&](int i1, int i2) {
//             if(imgPoints[i1].x > 0 && imgPoints[i2].x > 0) {
//                 cv::line(im, imgPoints[i1], imgPoints[i2], color, thickness);
//             }
//         };
...
//         // Draw label
//         if(cuboid.confidence > 0.5 && imgPoints[0].x > 0) {
//             string label = cuboid.class_name + " " + std::to_string(int(cuboid.confidence * 100)) + "%";
//             int baseline = 0;
//             cv::Size textSize = cv::getTextSize(label, cv::FONT_HERSHEY_SIMPLEX, 0.5, 1, &baseline);
//             cv::Point textOrg(imgPoints[0].x, imgPoints[0].y - 5);
...
//             // Draw text
//             cv::putText(im, label, textOrg, cv::FONT_HERSHEY_SIMPLEX, 0.5, 
