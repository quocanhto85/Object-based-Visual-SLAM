...
void MapDrawer::SetCuboidsForFrame(unsigned long frameId, const std::vector<ORB_SLAM3::Cuboid>& cuboids)
{
...
// Backward compatibility method - converts old vector-based format to map-based
void MapDrawer::SetCuboids(const std::vector<std::vector<ORB_SLAM3::Cuboid>>& cuboids)
{
    ...
    // Store each frame with sequential frame IDs
    for(size_t i = 0; i < cuboids.size(); i++)
    {
        mCuboidMap[i] = cuboids[i];
    }
    
    // If too many frames, keep only the last MAX_STORED_CUBOID_FRAMES
    if(mCuboidMap.size() > MAX_STORED_CUBOID_FRAMES)
    {
        auto it = mCuboidMap.begin();
...
void MapDrawer::ClearOldCuboids(int keepLastNFrames)
{
    unique_lock<mutex> lock(mMutexCuboids);
